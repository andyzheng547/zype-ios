...
@property (weak, nonatomic) IBOutlet UIWebView *subscriptionDisclaimerWebview;
...
- (void)configureController {
    ...
    [self setupSubscriptionDisclaimer];
    
    [self requestProducts];
}

- (void)setupSubscriptionDisclaimer {
    ...
    self.subscriptionDisclaimerWebview.delegate = self;
    ...
    NSString *styledDisclaimer = [NSString stringWithFormat:@"<style type=\"text/css\">a {color: #%@;}</style>%@", [UIUtil hexStringWithUicolor:brandColor], disclaimerText];

    htmlString = [NSString stringWithFormat:htmlString, @"", styledDisclaimer, nil];
    [self.subscriptionDisclaimerWebview loadHTMLString:htmlString baseURL:nil];
}
